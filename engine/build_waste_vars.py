...
def build(case: str):
    ...
    # (나)생활 = **가정계만** — 천안 골든 대조로 확정 (652≈665: 판 차이 수준 · 포함이면 845
    #  라 배제. 09-07 distill). 골든 원천은 통계연보(2022기준)·우리는 전국폐기물 2024 —
    #  값 차이는 원천·판 차이(F-4)이고 출처 주석 문제는 _확인필요 참조.
    ...
    if wj_path.exists():
        ...
        # 관리구역 표 — 면적·인구·세대 + **배출량(C) = 성상 4종 발생량 합** (천안 골든 실측:
        # 4,946 = 652+1,714+2,384+196 — 09-07 distill). 처리량(D)=C·수거율 100.0.
        # 면적은 소수 2자리(천안 골든 636.15 — 원주 정수와 1:1 갈림, 원자료 정밀도 보존 쪽).
        ...
        if gwan and gwan.get("행"):
            ...
            for k, *_ in BASE_MAP:
                tabs2 = wj["값"].get(k)
                if tabs2:
                    t2 = list(tabs2.values())[0]
                    i2, _ = cols(t2)
                    발생합 += round(float(str(t2["행"][0][i2]).replace(",", "")) / 365)
            배출s = f"{발생합:,}" if 발생합 else None
            성상["관리구역"] = {"앵커": "행정구역(A)", "skip": 0,
                            "행": [[sigun, 면적, 인구, 면적, 인구, "100.0",
                                  배출s, 배출s, "100.0" if 발생합 else None]]}
            통계추가["인구"] = int(g[3])
            통계추가["세대"] = int(g[5])
    ...
    for item, why in [("성상 표 출처 주석", "베이스 문구가 '통계연보' 고정인데 값 원천은 전국폐기물 2024 — 통계연보연도 [확인 필요] 유지, 문구 토큰화(spec)는 후속 제안. F-4·G-5 연계"),
