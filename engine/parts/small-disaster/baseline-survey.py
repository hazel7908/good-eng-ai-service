...
def build_slots(v):
    ...
    # josa(None) 은 첫 후보를 돌려줘 값 없는 자리에 반쪽 조사가 실린다 — 0600·0711 과 같은 가드
    out["표토_조사"] = josa(t["표토"], "으로로") if t.get("표토") else MISSING
    sl = v.get("사면", {})
...
def build_tables(hwp, v):
    ...
    if rows:
        ...
        # 🚨 (09-03 당시) `blank_row` 가 한 열만 지워 나머지 칸에
        #    기준 사업 값이 그대로 남는다 — 충주 3장에서 `0.110, 0.162` 가 실제로
        #    남아 표유출검사 ③에 걸렸다 — 이후 util 이 행 전체 순회로 고쳐졌지만(09-09 확인)
        #    이 표는 행 구성 자체가 사업 고유라 `blank_table_here` 유지.
        # ⚠️ 머리는 **1행**(두 줄 셀 `산정\n지점` — 행이 아니라 셀 안 문단이다).
    ...
    if yrs:
        ...
        # 🚨 `list(dict)` 는 **키**를 준다 — 값이 아니다. 그래서 데이터 행 11개에
        #    `연도|평균|최고|최저|강수량|강수일|습도` 가 그대로 찍혔다 (충주 실측 09-08).
        # ⚠️ 게다가 **dict 순서 ≠ 표 열 순서**다. 표는 실측상
        #    `연도 | 기온 평균·최고·최저 | 상대습도 | 풍속 | 강수량` 이고
        #    vars 는 `연도·평균·최고·최저·강수량·강수일·습도·풍속` 이다. 명시로 짚는다.
        COL = ("연도", "평균", "최고", "최저", "습도", "풍속", "강수량")
        for i, row in enumerate(yrs):
            vals = [row.get(k) for k in COL] if isinstance(row, dict) else (
                list(row) + [None] * (7 - len(row)))
            W("평  균", 1 + i, 0, vals)
        # 평균 행 — 연도 행 **다음**. vars 는 기온·습도·풍속·강수량 넷만 준다.
        # ⚠️ 최고·최저 평균은 vars 에 없다 — 연도 행에서 계산해 넣지 않는다(환각 금지).
        W("평  균", 1 + len(yrs), 0,
          ["평 균", 기상.get("평균기온"), None, None,
           기상.get("평균습도"), 기상.get("평균풍속"), 기상.get("평균강수량")])
        # 🚨 월별 12행은 vars 에 자료가 없다 — **건너뛰면 기준 사업(천안) 값이 그대로 남는다.**
        #    자료가 없으면 비운다(rule §6-3). 되먹임에선 안 보이고 다른 사업에서만 드러난다.
        # ⚠️ 월별은 12행인데 표 마지막 행(12월)이 하나 더 밀려 있다 — 13으로 돈다.
        #    12 로 두면 **12월 행만 천안 값이 남는다**(실측 09-08).
        for k in range(13):
            W("평  균", 2 + len(yrs) + k, 0, [None] * 7)
    else:
    ...
    # 🚨 앵커 `B등급` 은 **데이터 행 값**이라 그 행부터만 지워진다 — 위의 1·2행이 살아남아
    #    천안 교량명(`남천안IC1교`·`남천안IC교`)이 충주 산출물에 그대로 나갔다 (09-08 실측).
    #    머리행 라벨 `시설물 구분` 으로 잡아 표 전체를 비운다.
    _blank_all(hwp, "시설물 구분", 1)
