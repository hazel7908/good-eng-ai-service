...
BLANK = [("설치목적", 2, 2), ("연장", 2, 6),
         # ⚠️ 09-08 anchor_propose — 미처리 23표 중 사업 고유 5개.
         #    지번조서 14표는 아래 `토지소재지` 전용 호출이 이미 맡고 있다.
         #    계획의 시행근거·전략환경영향평가 실시근거는 **법령표**라 고정.
         ("고호", 1, 1), ("요증설규격", 1, 1), ("개선방향 (개소)", 2, 1),
         ("개  선  방  향", 2, 1), ("관리계획", 1, 1)]
...
def build_tables(hwp, v):
    ...
    # 🚨 편입토지조서 14개(451행) — **소유자 이름**까지 든 사업 고유 표다.
    #    기본 상한 24행으로는 41행 조서를 다 못 비운다 (뒤 17행이 남는다).
    k = blank_tables(hwp, "토지소재지", 2, 16, max_rows=48)
    if k == 0:
        print("    WARNING: 앵커 '토지소재지' 못 찾음 — 편입토지조서 유출 위험")
