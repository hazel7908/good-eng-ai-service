...
BLANK = [("사업지구와의 이격거리", 1, 2), ("환경기준", 2, 2),   # 측정결과 2표뿐 (09-08 실측 — limit 3이면 3번째가 예측결과 표를 먹어
                       #   "기준치 만족여부" 6표와 이중 비움. ⚠️ 법령 대기환경기준 표는 셀이
                       #   `환 경 기 준`(공백 낀 표기)이라 이 앵커에 애초에 안 걸린다 — 안전)
         ("배출계수", 1, 3), ("기준치 만족여부", 3, 6),
