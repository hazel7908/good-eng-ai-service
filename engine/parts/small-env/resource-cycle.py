...
def build_slots(v):
    ...
    s = {
        ...
        "품셈연도": g(tj, "품셈연도"),
        "통계연보연도": g(tj, "통계연보연도"),
        # 성상 출처 주석 — vars 가 주면 그대로, 없으면 **기존 표기로 되돌아간다**
        # (원주 되먹임이 깨지지 않게. 값이 들어오면 전국폐기물 2024 로 정직하게 바뀐다).
        "성상출처_주석": (tj.get("성상출처_주석")
                       or (f"통계연보 {tj['통계연보연도']}. {sa.get('시군')}"
                           if tj.get("통계연보연도") and sa.get("시군") else MISSING)),
...
def build_tables(hwp, v):
    ...
    # 🚨 **시군 라벨을 앵커로 쓰면 안 된다** — 그건 **데이터 셀**이라 앵커 규약
    #    ("덮어쓰지 않는 머리행 라벨") 위반이다. 실제로 `현황조사내용` 표 조사범위 셀
    #    (`▪ 대상범위 : 천안시 및 사업계획지구`)에 표 안 `천안시` 가 하나 더 있어
    #    **skip 이 한 칸 밀렸고**, skip=4 가 건설 표에서 끝나 **지정폐기물 표에 원주
    #    `11|3|1|7` 이 남았다** (2026-09-01 Mac 표유출검사 적발).
    #    → 머리행 라벨로 교체. 셀 주소 실측: `행정구역(A)`=B1 · `처리방법별 발생량`=C1,
    #      데이터는 둘 다 **+2행**(부머리행 한 줄 있음), 라벨 A열은 `{{시군}}` 치환분이라 보존.
    for anchor, sk, lab in [("행정구역(A)", 0, "관리구역"),
                            ("처리방법별 발생량", 0, "생활계"),
                            ("처리방법별 발생량", 1, "사업장"),
                            ("처리방법별 발생량", 2, "건설"),
                            ("처리방법별 발생량", 3, "지정폐")]:
        if not blank_row(hwp, anchor, 2, keep_first=1, skip=sk):
            print(f"    WARNING: 성상별 {lab} — 앵커 '{anchor}'(skip={sk}) 못 찾음")
