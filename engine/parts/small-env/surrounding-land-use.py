...
from hwp_util import (MISSING, col_begin, down, find_in_table, fit_rows, fr,
...
def build_slots(v):
    ...
    return {
        ...
        "통계연보연도": g(tj, "통계연보연도"),
        "시군": g(v.get("사업", {}), "시군"),
        # 표 캡션 `{{시군}} 및 {{읍면}}의 지목별…` 에 쓴다 (2026-09-07 spec 추가).
        "읍면": g(v.get("사업", {}), "읍면"),
    }
...
def build_tables(hwp, v):
    ...
    #    라벨 칸은 이미 이 표들을 다루는 핸들러가 한 줄씩 쓴다.
    사 = v.get("사업", {})
    시군, 읍면, 리 = 사.get("시군"), 사.get("읍면"), 사.get("리")
    W("지적면적", 1, 0, ["".join(x for x in (시군, 읍면, 리) if x) or None])   # A=세로 병합 라벨
