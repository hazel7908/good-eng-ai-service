...
"""
from hwp_util import MISSING, josa, write_at
...
def build_slots(v):
    ...
    # josa(None) 은 첫 후보 조사를 돌려줘 값 없는 자리에 반쪽 조사가 실린다 — 0600 전례와 같은 가드
    out["조사지역_주소_조사"] = josa(s["조사지역_주소"], "으로로") if s.get("조사지역_주소") else MISSING
    out["식생유형_지구_조사"] = josa(c["식생유형_지구"], "이가") if c.get("식생유형_지구") else MISSING
    yo = c.get("요약", {})
