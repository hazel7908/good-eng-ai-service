...
UNIT = {"전": (4.38, 1.400), "답": (4.24, 0.467), "과수원": (2.69, 0.630), "목장용지": (3.71, 0.295),
        ...
        "하천": (0.96, 0.027), "구거": (0.96, 0.027), "구": (0.96, 0.027), "유": (0.96, 0.027),
        "도": (12.42, 0.391),   # 별칭 — 총량계산 원장(천안 ㉛)이 유·구·도 축약 지목으로 집계
        "대지": (10.28, 0.600), "대": (10.28, 0.600),
...
def build_tables(hwp, v):
    ...
    # 🔬 셀 주소 실측(09-08): 합계 행 A 는 **구분~지목 3열 병합**이라  이 거기 선다.
    #     면 한 칸 밀려 지적면적 칸(D)을 건너뛰고 계산 필드에 맡겨져
    #    원주  가 살아남았다 (). 1 로 당겨 값으로 덮는다.
    W("지적면적", 1 + max(len(rows), 1), 1, [tot(2), tot(3), tot(4), tot(5), "-"])
