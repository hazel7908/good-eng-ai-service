...
def _decode_para_text(data: bytes) -> str:
    ...
    while i < len(data) - 1:
        ...
        # 🚨 **확장 제어문자는 8 WCHAR = 16바이트다** (제어코드 2B + 정보 12B + **꼬리 제어코드 2B**).
        #    예전엔 앞 2B 를 읽고 `i += 12`(합 14B)만 건너뛰어 **꼬리 제어코드에 커서가 걸렸고**,
        #    그것도 확장 제어라 판정돼 **또 14B 를 건너뛰며 진짜 글자를 먹었다.**
        #    실측(본환 토양 조사시기): `0002 … 0002 | 1차 : 2025.…` 에서 `1차 : 2` 7글자가
        #    사라져 골든이 `025.06.25 ~ …` 가 됐다 (Windows ㉒, 2026-09-04).
        ...
        if code < 0x0020:
            if code == 0x000A or code == 0x000D:  # 줄바꿈
                ...
                # ⚠️ **고정폭 빈칸(fwSpace)은 확장 제어가 아니라 `0x001F` 단일 문자다.**
                #    버리면 `2025. 06. 25` 가 `2025.06.25` 로 붙는다 — 눈에 안 띄는
                #    결손이라 spec 문자열이 조용히 어긋난다. 공백으로 살린다.
                chars.append(" ")
            # 나머지 제어문자는 무시
            continue
