...
🚨 **이 도구는 과다 계상한다** (2026-09-08 실측). 되먹임에서는 *핸들러가 vars 로
채운 표*와 *손도 안 댄 표*가 **똑같아 보인다** — 채운 값이 곧 기준 사업 값이기 때문이다.
소환에서 82건이 잡혔는데 `table_leak` 을 천안 산출물에 돌리니 **유출 0** 이었다
(남은 경고는 능선축 위계·비탈면 표준경사 같은 고정 기준표).
→ ⚠️ 는 **후보 목록**이지 결함 수가 아니다. 확정은 **다른 사업 산출물의 `table_leak`**
  으로만 한다. 이 도구가 쓸모 있는 곳은 채움 코드가 거의 없는 **C 단계 유형**
  (본환·전략)이다 — 거기서는 안 채운 표가 곧 손 안 댄 표다.
"""
