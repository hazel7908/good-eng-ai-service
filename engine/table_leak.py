...
CATEGORY_BASE = {
    ...
    "strategic-env": {      # 기준: 충북 수산천·고명천 하천기본계획 (⑭ 수확 — rules/strategic-env/_category)
        "기준사업": "충북_수산천고명천",
        "시군": "제천시",
        "이름들": {"제천시", "수산면", "수산천", "고명천", "오티리", "고명리", "충청북도"},
        "상위": ("충청북도", "충북"),
    },
...
ALLOW_IDENTICAL = [
    ...
    # 토양분포현황(2) C열 `1.27~3.81` 등은 **TYPE A~D 침투율 상수**다 — 골든 2/2 동일.
    # (같은 표의 면적·구성비는 핸들러가 비운다 — 캡션 예외라도 그쪽은 값이 달라 잡힌다.)
    "토양분포현황(2)",
    # 0840 지목별 **연평균 발생부하 원단위** 14행 표 — 총량관리 기술지침 전국 상수다
    #    (전 4.38 · 답 4.24 · 체육용지 5.39 · 유원지 14.87 …, 원주=천안 전행 동일 실측 09-08).
    #    ⚠️ 캡션이 앞 문단인 `사업시행 전 토지계 발생부하량 산정` 으로 잡혀 `원단위`·`산정식`
    #    예외를 못 탔다 — 캡션이 표 자신의 것이 아닐 수 있다는 부류다.
    "토지계 발생부하량 산정",
