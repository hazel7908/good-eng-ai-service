...
def build(case):
    ...
    if 사회["주택"]:
        miss("주거_서술 '총가구수'", "골든 갈림 2:1 — 천안·괴산=일반가구수 ↔ 원주=주택수 합계. "
                              "다수+의미(가구≠주택)로 일반가구수 채택 — 원주 되먹임 채점에서 이 문장은 정답지 오류로 분류")
        miss("세대당 인구 자릿수", "원주·천안 1자리 ↔ 괴산 2자리 (2:1, 1자리 기본). 괴산 골든 2019 값 1.80은 계산(1.89)과도 다름 — 정답지 오류 의심")
