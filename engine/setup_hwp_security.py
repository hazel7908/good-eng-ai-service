#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""한글 자동화 보안모듈 설치·등록 — ⚠️ **한글 2024 에서는 안 먹혔다. 참고용이다.**

   2026-09-08 실측: DLL 이름 2가지 × 위치 4곳 × 레지스트리 2경로를 다 맞춰도
   `RegisterModule("FilePathCheckDLL", …)` 이 **전부 False** 였다(한컴 포럼의
   같은 증상 스레드도 미해결). 실제 해법은 **작업 파일을 `%TEMP%` 에 두는 것**이다
   — 한글은 자기 임시 폴더 안 파일을 묻지 않고 연다(`generate.py` 가 그렇게 한다).
   이 스크립트는 "이 길은 막혔다"는 기록으로 남긴다.
