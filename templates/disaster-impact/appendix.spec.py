# -*- coding: utf-8 -*-
"""재평 8장 — **소재평 spec 준용** (2026-09-03 Mac). 재평 베이스 = 소재평 베이스(천안 서식) 파생.
...
빌더: 이 파일의 SPEC 은 소재평 spec 의 복사본 — src 가 같으므로 `templates/disaster-impact/appendix.hwpx` 로 같은
베이스가 한 번 더 빌드된다 (카테고리별 베이스 파일 규약). 차이 목록은 빌더가 무시한다.
"""
import importlib.util as _iu
import pathlib as _pl

_p = _pl.Path(__file__).resolve().parents[1] / "small-disaster" / "appendix.spec.py"
...
SPEC["재평_차이"] = [          # 사람 보완 자리 — fill-report 에 실린다 (핸들러 _확인필요 와 합쳐 본다)
    "재평 협의 서식 (실무지침 별지) — 소재평과 판 동일(고시 제2023-63호)",
]
