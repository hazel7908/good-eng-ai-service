# -*- coding: utf-8 -*-
"""재평 2장 — **소재평 spec 준용** (2026-09-03 Mac). 재평 베이스 = 소재평 베이스(천안 서식) 파생.
...
골격 일치 실측(횡성_조항리 재평 골든 대조): 골격 토큰 없음.
빌더: 이 파일의 SPEC 은 소재평 spec 의 복사본 — src 가 같으므로 `templates/disaster-impact/target-area.hwpx` 로 같은
베이스가 한 번 더 빌드된다 (카테고리별 베이스 파일 규약). 차이 목록은 빌더가 무시한다.
"""
import importlib.util as _iu
import pathlib as _pl

_p = _pl.Path(__file__).resolve().parents[1] / "small-disaster" / "target-area.spec.py"
...
SPEC["재평_차이"] = [          # 사람 보완 자리 — fill-report 에 실린다 (핸들러 _확인필요 와 합쳐 본다)
    "[표 2-3] 유형별 평가대상지역 (면적/선/점 개념) — 소재평과 같은 3분류",
    "전경사진(드론) + 현장사진 8 — 삽도 구성",
]
