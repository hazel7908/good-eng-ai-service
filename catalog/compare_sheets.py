#!/usr/bin/env python3
"""
삽도 — **골든셋 전수 대조**. 정답과 같은 베이스 위에서 우리 도구를 돌린다.
...
편입토지조서도 필요 없다.
    ...
    python catalog/compare_sheets.py                       # 지역개황도 전 사업
    python catalog/compare_sheets.py --kind 수계도
    python catalog/compare_sheets.py --site 청주_호명리
"""
...
def build_watercourse(site):
    """수계도 — 마커·하천 흐름 화살표·하천명·보호구역 채색.

    지역개황도와 달리 **지도 그림을 읽어야** 한다. 하천망 자료가 면형이라 물길을 못 뽑고,
    `hydro.py` 가 베이스의 하천 색을 읽어 화살표를 놓는다 (3-3)."""
    import map_fetch as M, hydro as Hy
    from pyproj import Transformer
    from PIL import Image

    base = os.path.join(SHEETS, site, "수계도.png")
    if not os.path.exists(base):
        return None, "베이스가 없습니다"
    g = json.load(open(GEOREF, encoding="utf-8")).get(site, {}).get("수계도")
    if not g:
    ...
    els = [{"type": "target", "at": anchor},
           {"type": "label", "at": [anchor[0], anchor[1] - 150],
            "text": "사업계획지구", "from": anchor}]
    try:
        mask = Hy.river_mask(im)
        arrows = Hy.flow_arrows(mask, anchor, grid=int(220 * ppm / 0.08))
        ...
        print(f"     [warn] {site} 하천 요소 — {type(e).__name__}: {e}")
    try:
        els += Hy.protected_zones(lon, lat, 0.05, anchor, ppm)
    except Exception as e:
        print(f"     [warn] {site} 보호구역 — {type(e).__name__}")
    els += [{"type": "scalebar", "at": [W - 470, H - 100],
             "length_px": int(4500 * ppm), "label": "4.50km"},
            {"type": "north", "at": [W - 160, H - 250]}]

    os.makedirs(OUT, exist_ok=True)
    spec = os.path.join(OUT, f"{site}.수계도.spec.json")
    json.dump({"base": base, "elements": els}, open(spec, "w", encoding="utf-8"),
              ensure_ascii=False)
    out = os.path.join(OUT, f"{site}.수계도.jpg")
...
def side_by_side(site, our, kind="지역개황도"):
    from PIL import Image
    suffix = "" if kind == "지역개황도" else f".{kind}"
    ans = os.path.join(ANSWERS, f"{site}{suffix}.jpg")
    ...
    p = os.path.join(OUT, f"{site}{suffix}.대조.jpg")
    c.save(p, quality=85)
    return p


def main():
    ap = argparse.ArgumentParser(description="삽도 전수 대조")
    ap.add_argument("--site", help="한 사업만")
    ap.add_argument("--kind", default="지역개황도", choices=["지역개황도", "수계도"])
    a = ap.parse_args()

    sites = [a.site] if a.site else sorted(ADDR)
    ok = 0
    for site in sites:
        our, err = (build_watercourse(site) if a.kind == "수계도" else build(site))
        if err:
            print(f"  ✗ {site:<12} {err}")
            continue
        cmp = side_by_side(site, our, a.kind)
        ok += 1
