...
- ⚠️ HWP 작업이 도는 중에 `taskkill` 을 겹치지 말 것. 장시간 작업은 **파이프 없이** 돌려야
  멈춤과 진행을 구분할 수 있다.
- 🚨 **`Open()` 이 산발적으로 멈춘다** (2026-09-08, 다섯 번 겪음). 끊어 보면 예외가
  `Open` 호출(`InvokeTypes(10000)`)에서 난다 — 표 편집이 아니라 **여는 중**이다.
  ⚠️ 원인을 아직 못 짚었다. 배제된 것: ①COM 파손 아님(직후 최소 시험 2.5초 통과)
  ②잔여 프로세스 아님(파트마다 정리해도 났다) ③사본 탓 아님(갓 복사한 파일 0.0초).
  프로세스당 **첫 `Open` 만** 2.5~49초로 들쭉날쭉하고 그 뒤로는 0.0초다 — 백신
  실시간 검사 쪽이 유력한 다음 후보다.
  → 실전 대처: **한 프로세스에서 문서를 이어서 연다**(두 번째부터 0.0초). 배치가
  파트마다 새 프로세스를 띄우면 기동 비용을 파트 수만큼 다시 문다. `engine/run_parts.py`
  는 파트마다 `Hwp.exe` 를 정리하고 프로세스를 새로 띄운다 — 안전하지만 느리다.
