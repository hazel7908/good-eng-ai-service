...
### ★ 찾을 문자열이 **문단 전체와 똑같으면** 실패한다 (2026-08-25 실증)

90자짜리 문단을 통째로 잡았더니 안 바뀌었다. 마지막 마침표 하나를 빼니 걸렸다.
**끝을 한 글자 남겨 둔다.**

### 표를 XML 에서 셀 때 걸리는 것 넷

`score_regional.py` 를 만들며 전부 실측했다.

| 함정 | 증상 | 처리 |
|---|---|---|
| **표 안의 표** | 비탐욕 정규식이 바깥 여는 태그와 **안쪽 닫는 태그**를 짝짓는다 | 깊이를 세어 최상위만 |
| **캡션에 태그** | `<hp:markpenBegin …/>공공하수처리시설 현황` — 형광펜이 걸린 캡션 | `<[^>]*>` 제거 |
| **삽도 액자** | 글자가 하나도 없는 표 = 그림 틀 | 셀 텍스트 0이면 제외 |
| **머리글·바닥글** | 이것도 표로 짜여 있다 | `<hp:header>`·`<hp:footer>` 먼저 걷어냄 |
