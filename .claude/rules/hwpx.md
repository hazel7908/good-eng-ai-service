...
```
### ⚠️ 세로 병합 때문에 행마다 셀 개수가 다르다 — 오른쪽부터 채운다

첫 열이 세로 병합된 표는 **그룹 첫 행만 칸이 하나 더 많다.**
지역개황 산업단지 표가 그렇다 — `구분`(일반/농공)이 그룹마다 병합돼 있어
행1·행10 은 6칸, 나머지는 5칸이다. 왼쪽부터 채우면 **한 칸씩 밀린다**
(실제로 `소재지` 자리에 면적이, `분양상태` 자리에 조성상태가 들어갔다).

→ `TableRowEnd` 로 **오른쪽 끝을 잡고 왼쪽으로** 채우면 병합과 무관하게 맞는다.
병합된 첫 칸은 손대지 않게 되므로, 그 열의 값은 별도로 다뤄야 한다.
...
지역개황 §B 표는 대부분(지목별·용도지역·도로·배출시설·문화재·하수·산업단지)이
이 구조라 **오른쪽 정렬이 기본**이다. 행별 칸 수는 베이스에서 직접 세어 확인할 것.
