...
- ⚠️ HWP 작업이 도는 중에 `taskkill` 을 겹치지 말 것. 장시간 작업은 **파이프 없이** 돌려야
  멈춤과 진행을 구분할 수 있다.
- 🚨 **배치는 파트마다 `Hwp.exe` 를 먼저 정리하고 시작한다** (2026-09-08 실측).
  정상 종료(`quit_hwp`)에 맡겼더니 **두 번째 파트부터 `Open()` 이 멈췄다** — 세 번 재현.
  강제로 끊으면 `Open()` 안에서 `RPC 호출 실패` 가 떠 정지 지점이 확정된다.
  `taskkill /F /IM Hwp.exe` → `_hwp_running()` 이 False 가 될 때까지 대기 → 실행.
  `sleep 3` 같은 고정 대기로는 모자라다. 도구: `engine/run_parts.py`.
