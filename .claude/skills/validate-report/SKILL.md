...
| **삽도 내용이 그 사업 것인가** ★ | **베이스(다른 사업) 그림이 그대로 남는다.** 텍스트 대조·빈칸 검사·정확도 어디에도 안 걸린다 |

### ⚠️ 삽도는 반드시 눈으로 본다 (2026-08-24)

베이스 문서는 **빈 양식이 아니라 다른 사업의 완성 보고서**다 (`_category.md` §5-1).
뚫지 않은 자리는 그 사업 내용이 남는데, **삽도는 BinData 라 어떤 텍스트 검사에도 안 걸린다.**

실제로 천안 지역개황에 **원주 삽도 14장**(수계모식도 `섬강 → 한강 · 34.54km`,
원주 정온시설 현장사진 8장)이 실려 나갔다. 정확도 65.2%·빈칸 잔여 0 을 통과한 상태였다.

**더 나쁜 것** — 고치려고 만든 검사가 수정과 **같은 매핑을 공유**해서
엉뚱한 그림을 지우고도 `삽도 잔존 없음 ✅` 을 냈다. PDF 를 보고서야 잡혔다.

| 규칙 | |
|---|---|
| 1 | **그림·레이아웃을 건드렸으면 PDF 육안 확인 전에 통과시키지 않는다** |
| 2 | **검사는 수정과 다른 근거를 쓴다.** 같은 가정을 공유하면 둘이 같이 틀린다 |
| 3 | 두 근거(예: 이름 매핑 ↔ 바이트·크기)가 **어긋나면 그것이 경보다** |

`generate.py` 의 `check_stale_figures()` 가 저장할 때마다 1·2를 자동 점검하지만,
**자동 점검이 통과했다는 것이 맞다는 뜻은 아니다** — 새 파트에서는 눈으로 한 번 확인할 것.
