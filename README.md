...
**6유형(환경 3·재해 3) 165파트 전부가 베이스+되먹임을 마쳤고, 소환 16/17 ·
소재평 6/8 · 재평 6/8 파트가 B(러프 생성·WRONG 0) 이상이다.** 커버리지 유형 균등 **76.4%** · 실무 건수 가중 **82.3%**
(10월 말 목표 75.3/82.9 — 사다리·측정은 `engine/coverage.py`). 법령 인용은 법제처 API
감시·자동 최신화 체계(law_scan→law_check→law_annex→law_update)가 가동 중이다.

### 첫 B 승급 (2026-09-08 — 천안 화덕리 러프 생성)

| 파트 | 채점 | 비고 |
|---|:-:|---|
| 0300 대상지역 | 63.6% · WRONG 0 | 형제 vars 승계 조립 |
| 0600 입지타당성 | 44.4% · WRONG 0 | 판정 기준 패턴 유지(2/2 실측) |
| 0726 자원순환 | 30.8% · WRONG 0 | calc_waste+0200 승계+성상 원자료 |
| 0400 주변토지 · 0724 토지이용 | 7.7 / 13.3% | 코드로 고칠 WRONG 0 — D2 문형 4:3 갈림(A-6) |

B 생산 라인: **승계 빌더**(`build_waste_vars`·`build_area_vars`·`build_case_skeleton`) +
**D2 정형 문장 조립**(문형은 골든 7건 전수 다수 계열) + **핸들러 dry-run**(빈-인풋에서
그럴듯한 값이 새는 결함을 생성 전에 적발 — 5건 실적). 아래 08-24 기준 파트별 상세는 유지.

---

### (이전) 2026-08-24 기준 — 첫 세 파트 완주 기록

**세 파트가 생성→검증 파이프라인을 완주했다.** 소음·진동과 대기질은 "코드로 고칠 수 있는 오류 0",
지역개황은 첫 생성을 마쳤다 (2026-08-24).
...
| 천안 화덕리 | **지역개황** | **OK 44/83항목 (53.0%)** · 코드 오류 0 — 표 37개 자동 채움 · 기준 사업 흔적 **0건** |
...
| 층 | 내용 | 상태 |
|:--:|---|---|
| A 본문 토큰 | 사업 고유값 | ✅ **베이스 문서 완성 · 토큰 29종** |
| **B 통계 표** | **자료 19종 → 절 19개** | ✅ **표 22개 자동 처리** (채움 17 · 비움 4 · 파생 1) |
| C 법령표 | 2.4 전체 | 고정 (98~99% 일치) |
| D 삽도 6종 | 지역개황도·수계도 등 | ⚠️ 생성은 되나 **hwp 삽입 미검증** — 현재 원주 삽도가 남는다 |
...
└── raw_data/                    ▓ git 제외 — 원본·옛 PoC(_archive-poc-2026-03, 참조 금지)·수확물
```
...
| [`docs/20260826_지역개황_구성요소.md`](docs/20260826_지역개황_구성요소.md) | **지역개황으로 만들어진 파일 70개 전수** — 어떤 파일이 무엇을 하는지 |
| [`docs/20260825_삽도_명세_지역개황.md`](docs/20260825_삽도_명세_지역개황.md) | **삽도 인계 명세** — 맥이 만들고 윈도우가 넣는다 |
| [`docs/20260824_결과보고_지역개황.md`](docs/20260824_결과보고_지역개황.md) | **지역개황 1차 결과 보고** — 천안 83항목 중 OK 40(48.2%), 결함을 성격별로 분류 |
| [`docs/20260825_윈도우_지역개황_베이스.md`](docs/20260825_윈도우_지역개황_베이스.md) | ✅ 완료한 Windows 세션 지시서 (베이스 + §B 표) |
...
| 3 | **나머지 파트 확대** — **지역개황 첫 생성 완료(08-25, 천안 OK 40/83·코드 오류 0)**. 남은 것은 판단이 필요한 6건(통계 판 고정·매립 표 계열·표 삽입·표 서식 변이·절 유무·절 도입 문장). 이후 수질 등 | 🔄 진행 중 (10월 말 전체 구현 목표) |
